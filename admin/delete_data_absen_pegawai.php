...
if(isset($_GET['id_absensi']) && is_numeric($_GET['id_absensi'])) {
    ...
    if ($result_delete) {
        ...
        echo '<script>window.location = "index.php?hal=data_absen_pegawai"</script>';
        exit();
    } else {
        "<script>
        Swal.fire({
            ...
            window.location = 'index.php?hal=data_absen_pegawai'; // Sesuaikan dengan halaman logout atau tindakan lainnya
        });
        </script>";
    }
} else {
